...
Grid::Grid(Vector2f size, Vector2f position, GameObject *object)
    : object(object) {
  // cout << "Start create grid" << endl;
  font.loadFromFile(FONT_PATH);
  text.setFont(font);
  text.setCharacterSize(floor(size.y));
  text.setPosition(position.x, position.y);
  text.setString("G");
  cout << "Text char size: " << floor(size.y) << " position: " << position.x << " " << position.y << endl;
...
void Grid::draw(RenderWindow &window) {
  window.draw(shape);
  cout << "Start draw text" << endl; 
  window.draw(text);
  cout << "Finish draw text" << endl;
}
...
void TextBox::shiftShowTextDown() {
  cout << "showTextIndex: " << showTextIndex
       << " allText.size(): " << allText.size()
       << " textCapacity: " << textCapacity
       << " showTextCapacity: " << showTextCapacity << endl;
...
CommandList::CommandList(Vector2f position, Vector2f size,
                         vector<string> allObject, int textCapacity, int showTextCapacity) {
  font.loadFromFile(FONT_PATH);
  shape.setPosition(position);
  shape.setSize(size);
  shape.setFillColor(COLOR1);
...
Gui::Gui() {
  ...
  MiniMapTexture.loadFromFile(
      "C:\\Users\\ADMIN\\Documents\\GitHub\\Cminusminus\\22676.jpg");
  MiniMap.setTexture(MiniMapTexture);
  MiniMap.setPosition(Vector2f(0, 705.6));
  MiniMap.setScale(Vector2f(0.617, 0.45));
}
...
void Gui::updateLayerSheet() {
  for (int i = 0; i < layerSheet->buttons.size(); i++) {
    if (i == layerSheet->buttons.size() - 1) // Over all button
      layerSheet->buttons[i].setDataMap(
          layerSheet->getData()->getOverAllLayer());
    else {
      layerSheet->buttons[i].setDataMap(
          layerSheet->getData()->layers[i]->insideLayer);
    }
...
void Gui::updateZoomButton() {
  ...
  if (zoomOut->getShape().getGlobalBounds().contains(mousePositionView)) {
    if (Mouse::isButtonPressed(Mouse::Left)) {
      cout << "zoom out" << endl;
      zoomOut->zoomOut(*map);
    }
...
void Gui::updateDetail() {
  if (map->getBorder().getGlobalBounds().contains(mousePositionView)) {
    ...
    detail->shiftShowTextUp();
  } else if (detail->getDown().getGlobalBounds().contains(mousePositionView)) {
    detail->shiftShowTextDown();
  }
}

void Gui::updateMapCenter() {
  if (key == Keyboard::Up) {
    Vector2i center = map->getCenter();
    Vector2i newCenter = Vector2i(center.x, center.y + 1);
    map->changeCenter(newCenter);
    ...
    Vector2i center = map->getCenter();
    Vector2i newCenter = Vector2i(center.x - 1, center.y);
    map->changeCenter(newCenter);
  ...
  while (window->pollEvent(event)) {
    ...
    if (event.type == Event::KeyPressed) {
      ...
      if (event.key.code == Keyboard::PageUp) {
        commandList->shiftShowTextUp();
      }
      if (event.key.code == Keyboard::PageDown) {
        commandList->shiftShowTextDown();
      }
    }
  }
}
